...
use crate::state::{Game, Seat, Win, WinType, TileType, SeatAccess};
...
impl TileIs for Meld {
    ...
    fn number(&self) -> Option<i8> { if self.is_seq() { None  } else { self.tiles[0].unwrap().number()} }
...
impl MeldHas for Meld {
    ...
    fn base_fu(&self) -> u8 {
        if self.is_seq() { 0 }
            ...
            2 // base value of a non-sequence meld
            * { if self.is_quad() { 4 } else { 1 }}         // quadrupled for quads.
            * { if self.has_simple() { 1 } else { 2 }} // doubled again for honor/terminal.
...
impl_MeldVecHas!(for Vec<Meld>, Vec<&Meld>, [&Meld], [Meld], Vec<Pair>, [Pair]);
